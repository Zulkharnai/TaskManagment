...
function Getdata() {
    ...
    ajax: {
        ...
        columns: [{
            ...
            data: 'task_id',
            render: (task_id) => {
                    return `<button class="btn btn-primary mx-1" data-toggle="modal" data-target="#TaskStoreModal" onclick="TaskEdit('${task_id}')" ><i class="fa fa-edit"></i></button><button class="btn btn-danger mx-1" onclick="TaskRemove('${task_id}')"><i class="fa fa-trash"></i></button>`;
                }
        }
    ]
    });
...
function TaskStore(){
    ...
    $.post("{{ route('TaskStore') }}", $('#TaskStoreForm').serialize())
    .done((res) => {
